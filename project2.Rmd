...
createLongTable <- function(df, isState=TRUE)
{
  ...
  result_long_sum <- result_long_sum %>%
    separate(Dates, into = c("Month","Day","Year"), # Create 3 separate columns
              remove = TRUE) %>%                    # remove Dates column
    mutate(Year.Month.Day = as.Date(ISOdate(year = paste0("20",Year),
                                            month = Month, day = Day))) 
}
...
# long_cali_confirmed <- cali_confirmed_df %>% 
#                 replace(is.na(.), 0) %>%                        
#                 summarise(across(c(12:ncol(cali_confirmed_df)),
#                             sum, na.rm=T)) %>%
#                 gather(key = Dates,
#                           value = Cases)

long_top3_cities <- createLongTable(top3_cities, isState=FALSE) %>%
    select(City, Year.Month.Day, Cases)
