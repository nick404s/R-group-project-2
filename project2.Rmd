...
format_numbers<- function(a_column, isDouble=TRUE)
{
  if (isDouble) 
  {
    ...
    a_column <- format(a_column, big.mark = ",")
  }
  a_column
}

# the shades of blue, grey, and red 
...
global_map <- leaflet(data = global_all_sums) %>%
  ...
  addCircleMarkers(lat = ~Lat.Mean, lng = ~Long.Mean, 
                   ...
                   color = ~qpal_confirmed(Confirmed), opacity = 0.8,group = "Confirmed",stroke=FALSE) %>%
  addCircleMarkers(lat = ~Lat.Mean, lng = ~Long.Mean, 
...
getSortedByCases<- function(df)
{
  ...
  result <- df%>%
          ...
          mutate_at(4,format_numbers, FALSE)%>% # format the cases
          select(Country=1, Counts=4)           # return the Columns
}

# Get sorted countries by confirmed cases and add the Rank column
global_confirmed_rank <-getSortedByCases(global_confirmed_sums) %>%
mutate(Rank = paste0(row_number(),"."), .before=Country) 
...
x_coord_vec <- as.Date(c("2020-02-19","2020-11-29", "2021-06-04", "2021-10-20"))
